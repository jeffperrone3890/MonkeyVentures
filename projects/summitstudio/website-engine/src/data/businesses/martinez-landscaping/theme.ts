 ...
 *  colors, fonts, border radius, and button style. One of several
 *  businesses in the Summit Studio template registry — see
 *  src/data/businesses/registry.ts for how the active business is selected.
 *  Components never hardcode hex values or palette-literal Tailwind
 *  colors; they reference the semantic role names defined here (primary,
 *  secondary, accent, background, surface, foreground, muted...) via
 *  tailwind.config.ts.
 *
 *  This is a build-time theme: tailwind.config.ts imports the active
 *  business's theme directly, so switching brands requires setting
 *  NEXT_PUBLIC_ACTIVE_BUSINESS and rebuilding — not a live, no-rebuild
 *  theme switch. (A CSS-variable-driven version would be the natural
 *  upgrade path if a live color-picker/generator UI is ever built.)
 * ─────────────────────────────────────────────────────────────────────────
 */
