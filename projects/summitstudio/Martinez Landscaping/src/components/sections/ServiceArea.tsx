    ...
    <Section id="service-area" tone="paper">
      <Container>
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
            ...
            <Reveal delay={0.1}>
              <ul className="mt-8 grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
                {SERVICE_TOWNS.map((town) => (
                  <li key={town.name} className="flex items-center gap-2 text-[15px] text-ink">
                    <MapPin className="h-4 w-4 shrink-0 text-fern" />
                    {town.name}
                  </li>
                ))}
              </ul>
          ...
          <Reveal from="right">
            <div className="relative aspect-[4/3] overflow-hidden rounded-5xl bg-forest shadow-lift">
              ...
              <div className="absolute inset-0 grid place-items-center text-center">
                <div>
                  <span className="relative mx-auto grid h-16 w-16 place-items-center rounded-full bg-amber text-forest shadow-lift">
